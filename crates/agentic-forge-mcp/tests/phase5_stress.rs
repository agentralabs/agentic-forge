...
fn extract_bp_id(result: &ToolCallResult) -> String {
    let text = match &result.content[0] { ToolContent::Text { text } => text.clone() };
...
async fn test_stress_rapid_blueprint_creation() {
    ...
    assert!(elapsed.as_millis() < 5000, "100 creates took {}ms", elapsed.as_millis());

    // Verify count
    let list_result = ToolRegistry::call("forge_blueprint_list", Some(json!({})), &s).await.unwrap();
    let text = match &list_result.content[0] { ToolContent::Text { text } => text.clone() };
    let data: serde_json::Value = serde_json::from_str(&text).unwrap();
...
async fn test_stress_full_pipeline() {
    ...
    let resolve = ToolRegistry::call("forge_dependency_resolve", Some(json!({
        "blueprint_id": bp_id
    })), &s).await.unwrap();
    assert!(resolve.is_error.is_none());

    // Add manual dependency
    let add_dep = ToolRegistry::call("forge_dependency_add", Some(json!({
        "blueprint_id": bp_id,
        "name": "custom-crate",
        "version": "0.5.0"
    })), &s).await.unwrap();
    ...
    let structure = ToolRegistry::call("forge_structure_generate", Some(json!({
        "blueprint_id": bp_id
    })), &s).await.unwrap();
    assert!(structure.is_error.is_none());

    // Skeleton creation
    let skeleton = ToolRegistry::call("forge_skeleton_create", Some(json!({
        "blueprint_id": bp_id
    })), &s).await.unwrap();
    assert!(skeleton.is_error.is_none());

    // Test generation
    let test_gen = ToolRegistry::call("forge_test_generate", Some(json!({
        "blueprint_id": bp_id
    })), &s).await.unwrap();
    assert!(test_gen.is_error.is_none());

    // Import graph
    let graph = ToolRegistry::call("forge_import_graph", Some(json!({
        "blueprint_id": bp_id
    })), &s).await.unwrap();
    assert!(graph.is_error.is_none());

    // Wiring
    let wiring = ToolRegistry::call("forge_wiring_create", Some(json!({
        "blueprint_id": bp_id
    })), &s).await.unwrap();
    assert!(wiring.is_error.is_none());

    // Validate
    let validate = ToolRegistry::call("forge_blueprint_validate", Some(json!({
        "blueprint_id": bp_id
    })), &s).await.unwrap();
    assert!(validate.is_error.is_none());

    // Export
    let export = ToolRegistry::call("forge_export", Some(json!({
        "blueprint_id": bp_id
    })), &s).await.unwrap();
    assert!(export.is_error.is_none());

    // Get final state
    let get = ToolRegistry::call("forge_blueprint_get", Some(json!({
        "blueprint_id": bp_id
    })), &s).await.unwrap();
    let text = match &get.content[0] { ToolContent::Text { text } => text.clone() };
    let data: serde_json::Value = serde_json::from_str(&text).unwrap();

    let elapsed = start.elapsed();
    assert!(elapsed.as_millis() < 5000, "Full pipeline took {}ms", elapsed.as_millis());

    // Verify populated
    assert!(data["entities"].as_u64().unwrap() >= 4, "Should have inferred entities");
    assert!(data["dependencies"].as_u64().unwrap() >= 3, "Should have dependencies");
    assert!(data["files"].as_u64().unwrap() >= 3, "Should have generated files");
    assert!(data["tests"].as_u64().unwrap() >= 5, "Should have generated tests");
}
...
async fn test_stress_rapid_list_calls() {
    ...
    for i in 0..20 {
        ToolRegistry::call("forge_blueprint_create", Some(json!({
    ...
    let start = Instant::now();
    for _ in 0..500 {
        let result = ToolRegistry::call("forge_blueprint_list", Some(json!({})), &s).await.unwrap();
        assert!(result.is_error.is_none());
    }
    let elapsed = start.elapsed();
    assert!(elapsed.as_millis() < 5000, "500 list calls took {}ms", elapsed.as_millis());
}
...
async fn test_stress_repeated_pipelines() {
    ...
    for i in 0..20 {
        ...
        ToolRegistry::call("forge_entity_infer", Some(json!({
            "blueprint_id": bp_id, "description": "users and posts"
        })), &s).await.unwrap();

        ToolRegistry::call("forge_dependency_resolve", Some(json!({
            "blueprint_id": bp_id
        })), &s).await.unwrap();

        ToolRegistry::call("forge_structure_generate", Some(json!({
            "blueprint_id": bp_id
        })), &s).await.unwrap();

        ToolRegistry::call("forge_test_generate", Some(json!({
            "blueprint_id": bp_id
        })), &s).await.unwrap();
    }

    let list = ToolRegistry::call("forge_blueprint_list", Some(json!({})), &s).await.unwrap();
    let text = match &list.content[0] { ToolContent::Text { text } => text.clone() };
    let data: serde_json::Value = serde_json::from_str(&text).unwrap();
...
async fn test_stress_rapid_updates() {
    ...
    let start = Instant::now();
    for i in 0..100 {
        ToolRegistry::call("forge_blueprint_update", Some(json!({
            "blueprint_id": bp_id,
    ...
    let elapsed = start.elapsed();
    assert!(elapsed.as_millis() < 2000, "100 updates took {}ms", elapsed.as_millis());
...
async fn test_stress_many_dependencies() {
    ...
    for i in 0..100 {
        ToolRegistry::call("forge_dependency_add", Some(json!({
            "blueprint_id": bp_id,
            "name": format!("dep-{}", i),
            "version": format!("{}.0", i)
        })), &s).await.unwrap();
