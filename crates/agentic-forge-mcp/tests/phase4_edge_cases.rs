...
async fn test_blueprint_update_invalid_status() {
    ...
    .await
    .unwrap();
    let text = match &create.content[0] {
...
fn test_tool_count_is_15() {
    ...
    assert_eq!(
        tools.len(),
        15,
        "Must have exactly 15 MCP tools, got {}",
        tools.len()
    );
}
...
fn test_tool_names_follow_forge_prefix() {
    for tool in ToolRegistry::list_tools() {
        assert!(
            tool.name.starts_with("forge_"),
            "Tool {} must start with 'forge_'",
            tool.name
        );
    }
}
