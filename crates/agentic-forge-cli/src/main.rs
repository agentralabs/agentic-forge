...
#[derive(Parser)]
#[command(name = "aforge", version, about = "AgenticForge — Blueprint Engine CLI")]
struct Cli {
...
enum BlueprintAction {
    Create { name: String, #[arg(long)] domain: String, #[arg(long, default_value = "")] description: String },
    Get { id: String },
    List { #[arg(long)] status: Option<String> },
    Validate { id: String },
    Export { id: String, #[arg(long, default_value = "json")] format: String },
    Delete { id: String },
    Update { id: String, #[arg(long)] name: Option<String>, #[arg(long)] description: Option<String> },
}

#[derive(Subcommand)]
enum EntityAction {
    Add { blueprint_id: String, name: String, #[arg(long, default_value = "")] description: String },
    Infer { blueprint_id: String, description: String },
    List { blueprint_id: String },
    Remove { blueprint_id: String, entity_id: String },
    Fields { blueprint_id: String, entity_name: String },
}

#[derive(Subcommand)]
enum DependencyAction {
    Resolve { blueprint_id: String },
    Add { blueprint_id: String, name: String, version: String },
    List { blueprint_id: String },
    Remove { blueprint_id: String, dep_id: String },
}
...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Commands::Serve { mode } => {
            if mode == "stdio" {
                agentic_forge_mcp::transport::run_stdio().await.map_err(|e| anyhow::anyhow!(e.to_string()))?;
            } else {
        ...
        Commands::Blueprint { action } => {
            match action {
                BlueprintAction::Create { name, domain, description } => {
                    let d = Domain::from_name(&domain).unwrap_or(Domain::Custom);
                    let id = engine.create_blueprint(&name, &description, d)?;
                    println!("{}", serde_json::json!({"blueprint_id": id.to_string(), "name": name}));
                }
                ...
                BlueprintAction::Update { id, name, description } => {
                    let bp_id: BlueprintId = id.parse().map_err(|e: String| anyhow::anyhow!(e))?;
                    if let Some(n) = name { engine.writer().rename_blueprint(&bp_id, n)?; }
                    if let Some(d) = description { engine.writer().set_description(&bp_id, d)?; }
                    println!("Updated {}", id);
                }
            }
        }
        Commands::Entity { action } => {
            match action {
                EntityAction::Add { blueprint_id, name, description } => {
                    let bp_id: BlueprintId = blueprint_id.parse().map_err(|e: String| anyhow::anyhow!(e))?;
                    let entity = Entity::new(&name, &description);
                    let eid = engine.writer().add_entity(&bp_id, entity)?;
                    println!("{}", serde_json::json!({"entity_id": eid.to_string(), "name": name}));
                }
                EntityAction::Infer { blueprint_id, description } => {
                    let bp_id: BlueprintId = blueprint_id.parse().map_err(|e: String| anyhow::anyhow!(e))?;
                ...
                EntityAction::List { blueprint_id } => {
                    let bp_id: BlueprintId = blueprint_id.parse().map_err(|e: String| anyhow::anyhow!(e))?;
                    let bp = engine.store.load(&bp_id)?;
                    for e in &bp.entities { println!("{} — {}", e.id, e.name); }
                }
                EntityAction::Remove { blueprint_id, entity_id } => {
                    let bp_id: BlueprintId = blueprint_id.parse().map_err(|e: String| anyhow::anyhow!(e))?;
                ...
                EntityAction::Fields { blueprint_id, entity_name } => {
                    let bp_id: BlueprintId = blueprint_id.parse().map_err(|e: String| anyhow::anyhow!(e))?;
                    let bp = engine.store.load(&bp_id)?;
                    let entity = bp.find_entity(&entity_name).ok_or_else(|| anyhow::anyhow!("Entity not found"))?;
                    for f in &entity.fields { println!("{}: {}", f.name, f.field_type.name()); }
                }
            }
        }
        Commands::Dependency { action } => {
            match action {
                DependencyAction::Resolve { blueprint_id } => {
                    let bp_id: BlueprintId = blueprint_id.parse().map_err(|e: String| anyhow::anyhow!(e))?;
                    let bp = engine.store.load(&bp_id)?;
                    let deps = agentic_forge_core::inventions::DependencyInferrer::infer(bp.domain, &bp.entities, &[]);
                    for dep in deps {
                ...
                DependencyAction::Add { blueprint_id, name, version } => {
                    let bp_id: BlueprintId = blueprint_id.parse().map_err(|e: String| anyhow::anyhow!(e))?;
                ...
                DependencyAction::List { blueprint_id } => {
                    let bp_id: BlueprintId = blueprint_id.parse().map_err(|e: String| anyhow::anyhow!(e))?;
                    let bp = engine.store.load(&bp_id)?;
                    for d in &bp.dependencies { println!("{} = \"{}\"", d.name, d.version); }
                }
                DependencyAction::Remove { blueprint_id, dep_id } => {
                    let bp_id: BlueprintId = blueprint_id.parse().map_err(|e: String| anyhow::anyhow!(e))?;
        ...
        Commands::Structure { action: StructureAction::Generate { blueprint_id } } => {
            let bp_id: BlueprintId = blueprint_id.parse().map_err(|e: String| anyhow::anyhow!(e))?;
            let bp = engine.store.load(&bp_id)?.clone();
            let files = agentic_forge_core::inventions::FileStructureGenerator::generate(&bp);
            for f in &files { println!("{}", f.path); }
        }
        Commands::Skeleton { action: SkeletonAction::Create { blueprint_id } } => {
            let bp_id: BlueprintId = blueprint_id.parse().map_err(|e: String| anyhow::anyhow!(e))?;
            let bp = engine.store.load(&bp_id)?;
            for entity in &bp.entities {
                println!("--- {} ---", entity.name);
                println!("{}", agentic_forge_core::inventions::SkeletonGenerator::generate(entity));
            }
        }
        Commands::Test { action } => {
            match action {
                ...
                TestAction::List { blueprint_id } => {
                    let bp_id: BlueprintId = blueprint_id.parse().map_err(|e: String| anyhow::anyhow!(e))?;
                    let bp = engine.store.load(&bp_id)?;
                    for tc in &bp.test_cases { println!("{} [{:?}]", tc.name, tc.test_type); }
                }
            }
        }
        Commands::Import { action: ImportAction::Graph { blueprint_id } } => {
            let bp_id: BlueprintId = blueprint_id.parse().map_err(|e: String| anyhow::anyhow!(e))?;
            let bp = engine.store.load(&bp_id)?;
            let edges = agentic_forge_core::inventions::ImportGraphGenerator::generate(&bp.files);
            for e in &edges { println!("{} -> {}", e.from_file, e.to_file); }
        }
        Commands::Wiring { action } => {
            match action {
                WiringAction::Create { blueprint_id } => {
                    let bp_id: BlueprintId = blueprint_id.parse().map_err(|e: String| anyhow::anyhow!(e))?;
                    let bp = engine.store.load(&bp_id)?;
                    let wirings = agentic_forge_core::inventions::WiringDiagramBuilder::build(&bp.entities, &bp.layers);
                    for w in &wirings { println!("{} -> {} ({:?})", w.source, w.target, w.wiring_type); }
                }
                WiringAction::List { blueprint_id } => {
                    let bp_id: BlueprintId = blueprint_id.parse().map_err(|e: String| anyhow::anyhow!(e))?;
                    let bp = engine.store.load(&bp_id)?;
                    for w in &bp.wiring { println!("{} -> {}", w.source, w.target); }
                }
            }
        }
        ...
        Commands::Concerns { domain } => {
            ...
            for c in &concerns { println!("{}: {:?}", c.name, c.concern_type); }
        }
        ...
        Commands::Config { action } => {
            match action {
                ConfigAction::Show => { println!("Default configuration"); }
